...
/** code -> { code, v, state, players:[{pid,name}], clients:Set, host, touched } */
const rooms = new Map();
...
const roster = room => ({ players: room.players.map(p => ({ pid: p.pid, name: p.name })), online: [...room.clients].map(c => c.pid) });
...
async function api(req, res, url) {
  ...
  if (seg[1] === 'health') return send(res, 200, { ok: true, rooms: rooms.size, up: Math.round(process.uptime()) });
